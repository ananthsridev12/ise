...
try {
    ...
    if (!$sender) {
        $sender = DB::fetchOne('SELECT * FROM kb_senders ORDER BY id LIMIT 1');
    }

    $techStack = DB::fetchAll('SELECT * FROM company_tech WHERE company_id = ? ORDER BY confidence DESC', array($companyId)) ?: array();

    $signalTypesRaw = $company['signal_types'] ?? '';
    $signalTypes    = $signalTypesRaw ? array_filter(array_map('trim', explode(',', $signalTypesRaw))) : array();
